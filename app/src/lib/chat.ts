...
/** Системный промпт. includeDay=false — когда вопрос САМОДОСТАТОЧЕН (обсуждение
 *  карты: данные уже в тексте вопроса) → небо дня НЕ добавляем, чтобы не тратить
 *  токены пользователя зря (правка владелицы 2026-07-07). */
export function systemPrompt(E: Engine, date: Date, tz: string,
  orb: number | ((name: string) => number) = 1.0, includeDay = true): string {
  const arche = db.archetypes.all().filter((a) => a.text).map((a) => `${a.object} — ${a.deity}: ${a.text}`).join('\n');
  const role = 'Ты — вдумчивый помощник практикующего астролога. Тебе даны УЖЕ ПОСЧИТАННЫЕ данные (позиции в знаках/градусах, мажорные аспекты с точными временами, события/дома). Не пересчитывай астрономию — доверяй этим числам и ТРАКТУЙ их, переводя на бытовой язык, простыми словами. Где уместно — опирайся на архетипы греческой мифологии, заложенные в программе по конкретным планетам. Не используй архетипы, не затрагивающие аспект. Пиши живо и образно, но БЫТОВЫМ языком — простыми словами, без заумных терминов и без отрыва от реальности. Пиши по-русски, ясно и по делу, структурируй ответ. Не пиши большие абзацы. Лучше короче, но яснее.';
  const day = includeDay ? `\n\n=== Данные дня ===\n${buildDayContext(E, date, tz, orb)}` : '';
  return `${role}${day}${arche ? `\n\n=== Архетипы (от астролога) ===\n${arche}` : ''}`;
}
...
export async function loadDeityMyth(key: string, object: string, deity: string): Promise<string> {
  const system = 'Ты — знаток мифологии и архетипов, помогаешь практикующему астрологу. '
    ...
    + 'параллели из других культур. По-русски, простым бытовым языком, без рассуждений о процессе. '
