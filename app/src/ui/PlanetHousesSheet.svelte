<script lang="ts">
  /** «Планеты в домах» — аккордеон по планетам; раскрытая строка показывает
   *  12 домов с темой и текстом из PLANET_HOUSE_LORE (пустые ключи пропускаются). */
  import { bottomSheet } from '../lib/sheet.ts';
  import { reveal } from '../lib/reveal.ts';
  import GlowCard from './GlowCard.svelte';
  import { LORE_PLANETS } from '../lib/planetSignLore.ts';
  import { PLANET_HOUSE_LORE } from '../lib/planetHouseLore.ts';
...
<section class="sheet glass" aria-label="Планеты в домах" use:bottomSheet={{ onclose }}>
    ...
    <GlowCard radius={14} selected={opened}>
      <div class="row reveal" class:opened use:reveal>
        ...
        {#if opened}
          <div class="body">
            {#each HOUSES as h (h)}
              {@const text = PLANET_HOUSE_LORE[`${planet}|${h}`]}
              {#if text}
                <div class="line">
