 ...
 *
 * Эталон — эталон А — с экрана профессиональной джйотиш-программой:
...
console.log('=== навамша D9 (эталон — таблица профессиональной программой) ===');
{
...
console.log('=== отношения планет ===');
{
  ...
  // ТАБЛИЦА АСТРОЛОГА (13.08.2026), присланная снимком: постоянные (наисаргика)
  // отношения семи грах. Переписана из его таблицы клетка в клетку — это ЭТАЛОН,
  // а не наша реконструкция классики. Проверяем все 42 упорядоченные пары разом:
  // строка = кто относится, и заодно что строка полна (6 других грах, без дыр).
  {
    const TABLE = {
      'Солнце':   { friend: ['Луна', 'Марс', 'Юпитер'], neutral: ['Меркурий'], enemy: ['Сатурн', 'Венера'] },
      'Луна':     { friend: ['Солнце', 'Меркурий'], neutral: ['Марс', 'Юпитер', 'Сатурн', 'Венера'], enemy: [] },
      'Марс':     { friend: ['Юпитер', 'Луна', 'Солнце'], neutral: ['Сатурн', 'Венера'], enemy: ['Меркурий'] },
      'Меркурий': { friend: ['Солнце', 'Венера'], neutral: ['Сатурн', 'Марс', 'Юпитер'], enemy: ['Луна'] },
      'Юпитер':   { friend: ['Солнце', 'Луна', 'Марс'], neutral: ['Сатурн'], enemy: ['Меркурий', 'Венера'] },
      'Венера':   { friend: ['Меркурий', 'Сатурн'], neutral: ['Марс', 'Юпитер'], enemy: ['Луна', 'Солнце'] },
      'Сатурн':   { friend: ['Меркурий', 'Венера'], neutral: ['Юпитер'], enemy: ['Марс', 'Луна', 'Солнце'] },
    };
    for (const [from, row] of Object.entries(TABLE)) {
      ok(`таблица астролога: ${from} — все шесть отношений`, () => {
        for (const [rel, list] of Object.entries(row)) {
          for (const to of list) assert.equal(naturalRelation(from, to), rel, `${from} → ${to}`);
        }
        assert.equal(row.friend.length + row.neutral.length + row.enemy.length, 6,
          `${from}: строка таблицы должна закрывать все шесть остальных грах`);
      });
    }
  }
  // ПРИМЕРЫ АСТРОЛОГА (13.08.2026) — его же разбор Юпитера и Сатурна, дословно.
  // Пришли ответом на просьбу перепроверить временные отношения; держим тестом,
  // чтобы правка таблиц не сломала то, что он подтвердил.
  ok('пример астролога: Сатурн в 4-м от Юпитера — нейтралитет становится дружбой', () => {
    assert.equal(naturalRelation('Юпитер', 'Сатурн'), 'neutral');
    assert.equal(temporalRelation(0, 3), 'friend');            // 4-й дом от Юпитера
    assert.equal(compoundRelation('neutral', 'friend'), 'friend');
  });
  ok('пример астролога: Сатурн в 5-м от Юпитера — нейтралитет становится враждой', () => {
    assert.equal(temporalRelation(0, 4), 'enemy');             // 5-й дом от Юпитера
    assert.equal(compoundRelation('neutral', 'enemy'), 'enemy');
  });
  // временное отношение ВЗАИМНО: 2-й от одной = 12-й от другой, обе половины
  // списка (2,3,4,10,11,12 против 1,5,6,7,8,9) зеркальны — проверяем все 12
  ok('временное отношение взаимно на всех двенадцати расстояниях', () => {
    for (let i = 0; i < 12; i++) assert.equal(temporalRelation(0, i), temporalRelation(i, 0));
  });
  // три строки колонки «Отношение» с экране профессиональной программы — сошлись все три
  {
...
console.log('=== Саде Сати ===');
{
  ...
  ok('эталон А (Луна Лев), Сатурн в Рыбах (2026) — аштама-Шани (8-й)', () =>
    assert.equal(sadeSati(4, 11)?.kind, 'ashtama'));
}
...
console.log('=== чувствительность даш ко времени рождения ===');
{
  ...
  // эталон А: Луна ~13,2°/сут, первая махадаша Венеры 20 лет
...
console.log('=== гочара от Луны ===');
{
  ...
  ok('эталон А (Луна Лев): Сатурн 2026 в Рыбах — 8-й от Луны, неблагоприятен', () =>
