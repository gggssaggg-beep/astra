...
<script lang="ts">
  ...
  /**
   * Кегль столбика планет. Ограничение ДВОЙНОЕ:
   *  · «удобный» кегль по числу строк (читаемость — чем меньше грах, тем крупнее);
   *  · потолок по ВЫСОТЕ клетки: столбик обязан в неё поместиться.
   *
   * Второе — не перестраховка. Треугольники севера тесные (вписанный круг вдвое
   * меньше, чем у центрального ромба), а совмещённый чертёж кладёт в клетку и
   * грахи гочары, и грахи рождения: четыре строки уже вылезали за треугольник и
   * налезали на соседний дом (поймано скриншотом экрана дня 2026-07-29).
   * CAP — высота столбика в единицах viewBox, полная высота ≈ n × 1.15 × fs.
   */
  const CAP_TIGHT = 11;   // треугольники севера
  const CAP_ROOMY = 20;   // центральные ромбы севера и квадраты юга
  const metrics = (n: number, rhomb: boolean) => {
    const base = n <= 1 ? 5.2 : n === 2 ? 4.8 : n === 3 ? 4.2 : n === 4 ? 3.6 : 3.1;
    const nice = rhomb || n < 3 ? base : base * 0.88;
    const fit = (rhomb ? CAP_ROOMY : CAP_TIGHT) / (Math.max(1, n) * 1.15);
    const fs = Math.min(nice, fit);
