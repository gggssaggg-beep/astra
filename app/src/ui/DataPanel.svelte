...
<section class="sheet glass sheet-base" aria-label="Данные и настройки" use:bottomSheet={{ onclose }}>
  ...
  <div class="block">
    ...
    <div class="hint small" style="margin-top:8px">
      ...
      {:else}Приходят западные аспекты по орбисам (☌ ⚹ □ △ ☍). Считается на тропическом
        круге с истинными узлами — теми же, что в западном списке дня.{/if}
      {#if nVedic !== vedic}
        <br /><b>Внимание:</b> школы разошлись — на экране {vedic ? 'джйотиш' : 'западная'},
        в уведомлениях {nVedic ? 'джйотиш' : 'западная'}. Так можно, считать будем честно
        по выбранной школе. Но тап по уведомлению откроет экран в режиме приложения — этих
        строк там может не оказаться.
      {/if}
    </div>
